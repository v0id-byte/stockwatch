...
## LLM 三臂 ablation 揭盲（2026-08-30，exploratory，判读规则预注册于 commit 8f9f9af）

MiniMax-M3 打分收官：**57,512/57,512 全部 ok**（重试后零失败；总耗 ~9.76M 输入 +
3.06M 输出 token，缓存永久化，prompt_version=ann_score_v1）。特征构建
`llm_event_features.parquet`（857 只，成员日 20 日窗事件覆盖率 53.6%）。

三臂同折同门禁结果（arm A 与主基线 walk-forward IC 逐位一致 0.027432——
确定性复现验证了 merge 未扰动管线）：

| 臂 | retro mean IC | decile spread | 年化超额 vs CSI500 | 状态 |
|---|---|---|---|---|
| A 纯价量 181 | +0.0274 | −0.0008 | −23.2% | REJECTED |
| B +3 计数列 | +0.0258 | −0.0009 | −22.5% | REJECTED |
| C +12 语义列 | +0.0238 | −0.0013 | −21.6% | REJECTED |

**判读（按冻结规则：语义增量需 C>B 且 C>A）：完全不成立，且方向相反——
C < B < A 单调递减。** 公告计数轻微负贡献（−0.0016，与既往"公告计数已被否定"
一致），LLM 语义分在其上再减 0.0020。结论：标题级 LLM 语义评分在 20d 周期、
CSI500 universe 上**没有可利用信息**，模型把这些列当噪声吸收后反而稀释了价量
信号。消息面假说（v1 标题级）被否定；若二期仍想追消息面，方向只剩：全文级
评分（Tier-2 语料的可得性偏差需先解决）、更短持有周期（PEAD 文献口径）、或
事件驱动单独建策略而非并入截面因子——三者都是新立项，不是现有框架内的调参。
