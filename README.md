...
- **Web AI 控制台**：网页里直接问行情、查个股、开始/停止持仓跟踪、设置/取消盯价，可不接飞书使用。
...
- **本地 Web 控制台**：展示最近运行、信号、持仓跟踪、盯价提醒和 5 日信号复盘，也能配置模型、渠道、开关和个性化。
- **社区因子入口**：可上传自定义因子文件作为本地贡献包，后续可提交给社区市场。
...
StockWatch is a family-oriented A-share alert bot for China A-share research. It monitors watchlists, tracked positions, buy-price alerts, announcements, news and market anomalies, then surfaces them in the local web console or sends Feishu/Lark alerts only when something deserves attention.
...
- Web AI console for stock lookup, market questions, position tracking and buy-price alerts without connecting Feishu/Lark.
...
- Local web console and signal quality report based on SQLite history.
