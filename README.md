...
Docker 下 Dashboard 与 daemon 共享同一份配置：所有服务都设置了 `STOCKWATCH_ENV_PATH=/root/.stockwatch/.env`，Dashboard 保存设置会写到这个共享文件，daemon/bot 容器需要 `docker compose up -d` 重新创建后才会读到最新值（现有进程仍持有启动时的环境变量）。
...
Web-only 模式下盘中轻量监控（`monitor_once`）仍会扫描并标记触价/触压状态，只是不发飞书卡片；可以在 Web 控制台的“活跃盯价提醒”里查看触发历史。Dashboard 的“因子市场”页也可直接在 UI 开启/关闭 `ENABLE_PROPAGATION`、`ENABLE_EVENTS` 和 `MARKET_REGIME`，无需手改 `.env`。
...
In Docker, the dashboard and the daemon share a single env file: every service sets `STOCKWATCH_ENV_PATH=/root/.stockwatch/.env`, and the dashboard persists settings to that path. Recreate the daemon/bot containers (`docker compose up -d`) after changing settings in the dashboard — already-running processes still hold the env vars from their original start-up.
...
In Web-only mode, `monitor_once` still scans and records triggered key-price alerts (it just skips the Feishu card); view trigger history in the dashboard's "Active price alerts" section. The Factor Marketplace page also lets you toggle `ENABLE_PROPAGATION`, `ENABLE_EVENTS` and `MARKET_REGIME` directly from the UI without editing `.env`.
